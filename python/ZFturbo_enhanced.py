...
def run_solution():
    ...
    while 1:
        ...
        # only small improvement: search destination + distance is almost unique for hotel_cluster
    ...
    while 1:
        ...
        s1_2 = (srch_destination_id, orig_destination_distance)
        if s1_2 in best_hotels_od_ulc:
            d = best_hotels_od_ulc[s1_2]
            topitems = nlargest(5, sorted(d.items()), key=itemgetter(1))
            for i in range(len(topitems)):
                if topitems[i][0] in filled:
                    continue
